...
def shrink_matrix(v_matrix, punct_indices):
    """
    Removes rows and columns corresponding to punctuation tokens.
    """
    v_matrix = np.delete(v_matrix, punct_indices, axis=0)
    v_matrix = np.delete(v_matrix, punct_indices, axis=1)
    return v_matrix

def shrink_human_tree(tree, punct_indices):
    """
    Removes punctuation nodes from the human Gold Standard tree
    and remaps the remaining nodes to contiguous indices (0, 1, 2...)
    so they align perfectly with the AI's shrunken tree.
    """
    all_nodes = sorted(list(tree.nodes()))
    valid_nodes = [n for n in all_nodes if n not in punct_indices]
    
    mapping = {old_idx: new_idx for new_idx, old_idx in enumerate(valid_nodes)}
    
    new_tree = nx.DiGraph()
    for n in valid_nodes:
        new_tree.add_node(mapping[n])
        
    for u, v in tree.edges():
        if u in mapping and v in mapping:
            new_tree.add_edge(mapping[u], mapping[v])
...
def run_pipeline(sentence, model_name="gpt2", human_baseline=None, remove_punctuation=True):
    """
    ...
    if remove_punctuation:
        for i in range(num_words):
            span = encoded_inputs.word_to_chars(i)
            if span is not None:
                word_text = sentence[span.start:span.end].strip()
                if word_text and all(c in string.punctuation for c in word_text):
                    punct_indices.append(i)
        if punct_indices:
            print(f"-> Removing punctuation at word indices: {punct_indices}")
            v_matrix = shrink_matrix(v_matrix, punct_indices)
            num_words = v_matrix.shape[0]
    ...
    return M_t, dependency_tree, v_matrix, punct_indices
